...
\[
\mathbf{v}^{t+1}_{j} = \mathbf{R}_1 \odot \mathbf{v}^t_{j} + \mathbf{R}_2 \odot (\mathbf{x}^t_{i} - \mathbf{x}^t_{j}) +\phi\mathbf{R}_3 \odot (\bar{\mathbf{x}}^t - \mathbf{x}^t_{j})
\]

and

\[
\mathbf{x}^{t+1}_{j} = \mathbf{x}^t_{j} + \mathbf{v}^{t+1}_{j}
\]
