...
from ..backend.operators import compute_pbest, compute_objective_function, compute_pairwise_best, compute_cso_velocity
from ..backend.topology import Star
from ..backend.topology import Pairwise
from ..backend.handlers import BoundaryHandler, VelocityHandler, OptionsHandler
from ..base import SwarmOptimizer
from ..utils.reporter import Reporter


class CSOMA(SwarmOptimizer):
    def __init__(
        self,
        n_particles,
        dimensions,
    ...
    def optimize(
        ...
        for i in self.rep.pbar(iters, self.name) if verbose else range(iters):
            ...
            # fmt: off
            self.swarm.center = np.mean(self.swarm.position, axis=0)  # calculate center all particles
            self.swarm.current_cost = compute_objective_function(self.swarm, objective_func, pool=pool, **kwargs)
            #self.swarm.pbest_pos, self.swarm.pbest_cost = compute_pairwise_best(self.swarm)
            self.swarm.winners, self.swarm.losers, self.swarm.pbest_pos, self.swarm.pbest_cost = compute_pairwise_best(self.swarm)
        ...
        final_best_cost = np.copy(self.swarm.best_cost)
