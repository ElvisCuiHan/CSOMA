...
def compute_pairwise_best(swarm):
    """Update the personal best score of a swarm instance
    ...
    try:
        ...
        dimensions = swarm.dimensions
        S = swarm.n_particles
        pairnum_1 = int(np.floor(S / 2))
        # Create a 1-D and 2-D mask based from comparisons
        randperm_index = np.random.permutation(S)  # randomly permuate all particle indexes
        rpairs = np.array([randperm_index[:pairnum_1], randperm_index[(S - pairnum_1):S]]).T  # random pair
        result = swarm.current_cost
        result0 = result[rpairs[:, 0]]; result1 = result[rpairs[:, 1]]
        cmask = result0 > result1
        # Apply masks
        losers = rpairs[:, 0] * cmask + rpairs[:, 1] * (~cmask)  # losers who with larger values
        winners = rpairs[:, 0] * (~cmask) + rpairs[:, 1] * (cmask)  # winners who with smaller values
        ...
        return (winners, losers, new_pbest_pos, new_pbest_cost)
...
def compute_cso_velocity(swarm, clamp, vh, bounds=None):
    """Update the velocity matrix
    ...
    try:
        # Prepare parameters
        swarm_size = swarm.position.shape
        winners = swarm.winners
        losers = swarm.losers
        phi = swarm.options["phi"]
        # Compute for cognitive and social terms
        first = (
            np.random.uniform(0, 1, swarm_size)[losers, :]
            * swarm.velocity[losers, :]
        )
        second = (
            np.random.uniform(0, 1, swarm_size)[losers, :]
            * (swarm.position[winners, :] - swarm.position[losers, :])
        )
        third = (
            phi
            * np.random.uniform(0, 1, swarm_size)[losers, :]
            * (swarm.center - swarm.position[losers, :])
        )
        # Compute temp velocity (subject to clamping if possible)
        temp_velocity = np.zeros(swarm_size)
        temp_velocity[losers, :] = first + second + third
...
def compute_csoma_position(swarm, bounds, bh):
    """Update the position matrix
    ...
    try:
        ...
        mutation_idx = np.random.choice(swarm.losers, 1)
        mutation_dim = np.random.choice(swarm.position.shape[1], 1)

        if bounds is not None:
            cache = np.array([np.array(bounds[0])[mutation_dim], np.array(bounds[1])[mutation_dim],
                              (np.array(bounds[0])[mutation_dim] + np.array(bounds[1])[mutation_dim]) / 2]).reshape(-1)
            temp_position[mutation_idx, mutation_dim] = np.random.choice(cache, 1)
            temp_position = bh(temp_position, bounds)
        else:
            minimum = np.min(temp_position)
            maximum = np.max(temp_position)
            cache = np.array([np.array([minimum] * swarm.position.shape[1])[mutation_dim],
                              np.array([maximum] * swarm.position.shape[1])[mutation_dim],
                              (np.array([minimum] * swarm.position.shape[1])[mutation_dim] +\
                               np.array([maximum] * swarm.position.shape[1])[mutation_dim]) / 2]).reshape(-1)
            temp_position[mutation_idx, mutation_dim] = np.random.choice(cache, 1)
